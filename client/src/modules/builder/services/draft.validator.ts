...
export function validateDraft(draft: QuizDraft): DraftErrors {
  ...
  for (const q of draft.questions) {
    ...
    if (q.type === 'mcq') {
      ...
      if (q.options.length < 2) e.options = 'Add at least two choices';
      else if (Object.keys(e).some((k) => k.startsWith('options.'))) e.options = 'Every option needs text or an image';
      else if (!q.options.some((o) => o.key === q.correctKey)) e.options = 'Select the correct answer';
    }
