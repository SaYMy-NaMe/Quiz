...
import { SERVER_ORIGIN } from '@/utils/baseURL';
...
const questions = [{ id: 'q1', type: 'mcq', prompt: '2+2', promptType: 'text', required: true, options: [{ id: 'o1', text: '4' }, { id: 'o2', text: '5' }], points: 1 }];
...
function fakeFetch(input: RequestInfo | URL, init?: RequestInit): Promise<Response> {
  ...
  // Requests are absolute (BASE_URL); normalise to the API-relative path.
  const url = raw.startsWith(SERVER_ORIGIN) ? raw.slice(SERVER_ORIGIN.length) : raw;
  ...
  if (url === `/api/quizzes/v/${TOKEN}`) return json({ quiz });
  ...
  if (url.endsWith('/submit')) {
    ...
    return json({ receipt: { submissionId: 'sub1', score: 1, maxScore: 1, needsReview: false, durationSeconds: 3, submittedAt: now(), reason: (body as { reason: string }).reason } });
  }
  if (url.endsWith('/result')) return json({ receipt: { submissionId: 'sub1', score: 1, maxScore: 1, needsReview: false, durationSeconds: 3, submittedAt: now(), reason: 'manual' } });
...
describe('examinee flow from a copied share link (no session)', () => {
  ...
  it('opens Step 1, starts the timed exam on "Start Quiz", records a violation, submits and releases the lock', async () => {
    ...
    expect(calls[0]).toMatchObject({ method: 'GET', url: `/api/quizzes/v/${TOKEN}` });
