...
export function Component() {
  ...
  useEffect(() => {
    ...
    navigate(persisted.receipt ? examineePaths.result(token) : examineePaths.test(token), { replace: true });
  }, [navigate, quiz.id, token]);
  ...
  const submit = async (examinee: ExamineeRecord) => {
    ...
    setFieldErrors(null);
    try {
      const { examinee: clean } = await examineeApi.validate(token, examinee);
      useAttemptStore.getState().setPendingExaminee(token, quiz.id, clean);
      navigate(examineePaths.test(token));
    } catch (err) {
