...
/** Step 1 — instructor-defined metadata form. No timer, no proctoring, nothing created server-side. */
export function ExamineeDetailsForm({ quiz, initialValues, busy, serverError, serverFieldErrors, onSubmit }: Props) {
  const [values, setValues] = useState<FormValues>(() => Object.fromEntries(Object.entries(initialValues ?? {}).map(([k, v]) => [k, String(v)])));
  ...
  return (
    <form onSubmit={submit} noValidate>
      <section className="gf-card">
        ...
        />
        <p className="small muted" style={{ marginBottom: 0 }}><span className="gf-required-dot">*</span> Required</p>
      </section>
