...
export function DynamicFormRenderer({ fields, values, errors, lockedFieldIds = [], onChange, onBlur }: Props) {
  return (
    <>
      {fields.map((f) => {
        const id = `ex-${f.fieldId}`;
        const error = errors[f.fieldId];
        const locked = lockedFieldIds.includes(f.fieldId);
        const common = {
          id,
          name: f.fieldId,
        ...
        return (
          <div className="field" key={f.fieldId}>
            ...
            {locked && <span className="field__hint">Pre-filled from your invitation.</span>}
            {error && (
