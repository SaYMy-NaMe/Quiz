...
interface EditorState {
  ...
  addQuestion: (type?: DraftQuestion['type']) => void;
...
export const useEditorStore = create<EditorState>((set) => {
  ...
  return {
    ...
    addQuestion: (type = 'mcq') =>
      set((s) => ({ dirty: true, draft: { ...s.draft, questions: [...s.draft.questions, DraftFactory.question(type)] } })),
    removeQuestion: (key) =>
    ...
    addOption: (qk) =>
      patchQuestion(qk, (q) => ({ ...q, options: [...q.options, DraftFactory.option()] })),
    removeOption: (qk, ok) =>
      patchQuestion(qk, (q) => {
        if (q.options.length <= MIN_MCQ_OPTIONS) return q;
