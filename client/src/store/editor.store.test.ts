...
describe('editor store', () => {
  ...
  it('allows any number of options (min 2) and keeps the answer key valid', () => {
    ...
    for (let i = 0; i < 10; i++) state().addOption(q.key);
    expect(state().draft.questions[0]!.options).toHaveLength(12);
  ...
  it('maps drafts to the wire payload with correctIndex and back from a quiz', () => {
    ...
    const quiz: Quiz = {
      id: 'q', ownerId: 'o', title: 'Mapped', description: '', status: 'draft', shareToken: null,
      ...
      questions: [{ id: 'q1', type: 'mcq', prompt: 'Pick', promptType: 'text', required: true, options: [{ id: 'o1', text: 'A' }, { id: 'o2', text: 'B' }], correctOptionId: 'o2', points: 1 }],
      createdAt: '', updatedAt: '',
    };
