import { Router } from 'express';
import type { LeaderboardService } from './leaderboard.service';
import type { ShareService } from '@/modules/share';
import { resolveShare } from '@/modules/share';
import { requireInstructor } from '@/modules/auth';

/** Instructor endpoint mounted at `/api/quizzes/:id/leaderboard`. */
export function createInstructorLeaderboardRouter(leaderboard: LeaderboardService): Router {
  const router = Router({ mergeParams: true });
  router.use(requireInstructor);
  router.get('/', (req, res) => {
    const { id } = req.params as Record<string, string>;
    res.json({ leaderboard: leaderboard.forInstructor(req.instructor!.id, String(id)) });
  });
  return router;
}

/** Examinee endpoint mounted at `/api/share/:token/leaderboard`. */
export function createPublicLeaderboardRouter(share: ShareService, leaderboard: LeaderboardService): Router {
  const router = Router({ mergeParams: true });
  router.get('/', resolveShare(share), (req, res) => {
    res.set('Cache-Control', 'no-store');
    res.json({ leaderboard: leaderboard.forExaminee(req.share!) });
  });
  return router;
}
