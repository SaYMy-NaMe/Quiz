 ...
 */
export function useProctor({ token, invite, active, onThresholdReached }: Options) {
  ...
  const handle = useCallback(
    (event: ViolationEvent) => {
      ...
      void proctorApi
        .report(token, state.attempt.id, event.kind, invite)
        .then((r) => {
    ...
    [token, invite, onThresholdReached],
  );
